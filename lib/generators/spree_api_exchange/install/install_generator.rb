    ...
    class InstallGenerator < Rails::Generators::Base
      ...
      def add_javascripts
       frontend_js_file = "app/assets/stylesheets/spree/frontend.js"
        ...
        if  File.exist?(frontend_js_file)
          append_file frontend_js_file, "//= require spree/frontend/spree_api_exchange\n"
        end
      ...
      def add_stylesheets
        ...
        if  File.exist?(frontend_css_file)
          inject_into_file frontend_css_file, " *= require spree/frontend/spree_api_exchange\n", :before => /\*\//, :verbose => true
      ...
      def add_migrations
        run 'bundle exec rake railties:install:migrations FROM=spree_api_exchange'
      end
